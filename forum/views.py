...
def log(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(request, "Пользователя или пароль неверные!")
    ...
    if form.is_valid():   
        ...
        if user is not None:
            if user.is_active:
                ...
                messages.error(request, "Недействительный пользователь!")
...
def update_password(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Введите пароль правильно!')
    else:
...
def discussao(request, discussao_titulo):
    ...
    pesquisa = request.GET.get("Поиск")
    if pesquisa:
    ...
    #caso nao haja nada na barra de pesquisa, mostra todos os objetos Topico  
        ...
        try:
            ...
            raise Http404("Темы не существует")
...
def topico(request, discussao_titulo, topico_id):
    ...
    try:
        ...
        raise Http404("Тема не существует в этом обсуждении")
